...
def group_compute(sent_list):
    """
    Evaluate surprisal on each sentence independently. Zip results if necessary.

    --> Note: only needed for string textbox input (not file input)
    """
    grouped_indices = [(i, i+1) for i in range(len(sent_list))]  # Each sentence is a group of its own

    # Compute surprisal for each sentence independently
    group_files = []
    group_number = 1 # for naming each result with a counter (results_#.tsv)
    for group in grouped_indices:
        print(f"computing next sentence ({group_number} out of {len(grouped_indices)}) in indices: {group}")
        start, end = group
